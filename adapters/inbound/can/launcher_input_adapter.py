...
class CANLauncherInputAdapter(LauncherInputPort):
    ...
    def on_message(self, msg: can.Message):
        try:
            ...
            can_id, converted_data = handler(msg)
            for callback in self._subscribers:
                callback(can_id, converted_data)
        except KeyError:
            print(f"No handler for {msg.arbitration_id}")
            return
        
    def on_current_angle_feedback(self, msg: can.Message) -> Tuple[int, AnglePacket]:
        data = msg.data
        if len(data) == 6:
            ...
            return msg.arbitration_id, AnglePacket(angle, direction)
        else:
    ...
    def on_distance_feedback(self, msg: can.Message) -> float:
        data = msg.data
        if len(data) == 4:
            return msg.arbitration_id, struct.unpack("<f", data)[0]
        else:
            raise ValueError("Invalid distance feedback")
        
    def on_ammo_status(self, msg: can.Message) -> Tuple[int, List[bool]]:
        ...
        return msg.arbitration_id, flag1 + flag2 + flag3